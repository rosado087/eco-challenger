...
namespace EcoChallenger.Controllers
{
    public class ChallengeController : BaseApiController
    {
        ...
        /// challenges of the user</returns>
        [Authorize(Roles = "Admin")]
        [HttpPost("EditChallenge")]
        public async Task<JsonResult> EditChallenge([FromBody] ChallengeModel challengeModel, int id)
        {
            try{
                ...
                var challengeTitle = await _ctx.Challenges.FirstOrDefaultAsync(c => c.Title == challenge.Title);
                if(challengeTitle != null){
                    return new JsonResult(new{ success = false, message = "Já existe um desafio com este título"});
                }

                var challengeDescription = await _ctx.Challenges.FirstOrDefaultAsync(c => c.Description == challenge.Description);
