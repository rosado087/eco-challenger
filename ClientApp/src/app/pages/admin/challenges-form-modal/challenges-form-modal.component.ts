...
export class ChallengeFormModalComponent implements OnInit {
  ...
  doStateChange = effect(() => {
      ...
      untracked(() => {
          const elem = this.challengeModalRef() as ElementRef<HTMLDialogElement>
  ...
  loadChallenge(id: number) {
          this.netApi
              .get<GetChallengeResponse>('Challenge', 'GetChallenge', undefined, id.toString())
              .subscribe({
                  next: (data) => {
                      if (!data)
                          this.formError =
  ...
  createChallenge(formData: FormData) {
    this.netApi.post<SuccessModel>('Challenge', 'CreateChallenge', formData).subscribe({
      next: (data) => {
        if (!data.success)
          this.formError = data.message ?? 'Erro ao guardar o desafio.'
        else {
          this.formError = null
  ...
  this.netApi
    .post<SuccessModel>('Challenge', 'EditChallenge', formData, this.challengeId()!.toString())
    .subscribe({
      next: (data) => {
        if (!data.success)
          this.formError = data.message ?? 'Erro ao guardar o desafio.'
