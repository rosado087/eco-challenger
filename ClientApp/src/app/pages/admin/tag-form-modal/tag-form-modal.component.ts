...
export class TagFormModalComponent implements OnInit {
  ...
  tagForm = new FormGroup({
    ...
    backgroundColor: new FormControl(this.loadedTag?.backgroundColor || '#FFDDDD', [
      Validators.required
    ]),
  ...
  loadTag(id: number) {
    ...
    .subscribe({
        next: (data) => {
            if (!data) this.formError = 'Não foi encontrada nenhuma tag com o ID correspondente.'
            else {
              this.loadedTag = data
              this.updateFormTagData(data)
            }
        },
        error: () => this.formError = 'Não foi encontrada nenhuma tag com o ID correspondente.'
    })
  }
  ...
  onImagePicked(event: Event): void {
    ...
    this.tagForm.get('icon')?.setValue(files[0], { emitModelToViewChange: false })
  }
  
  submitRecord() {
    this.tagForm.markAllAsTouched()
    if(!this.tagForm.valid || (!this.isIconValid() && this.tagForm.get('icon')?.value)) return
  ...
  createTag(formData: FormData) {
    this.netApi.post<SuccessModel>('Tag', 'create', formData)
    .subscribe({
      next: (data) => {
          if(!data.success) this.formError = 'Ocorreu um erro desconhecido ao guardar a tag.'
  ...
  editTag(formData: FormData) {
    this.netApi.post<SuccessModel>('Tag', 'edit', formData, this.tagId()!.toString())
    .subscribe({
      next: (data) => {
          if(!data.success) this.formError = 'Ocorreu um erro desconhecido ao guardar a tag.'
          else this.closeClick.emit()
