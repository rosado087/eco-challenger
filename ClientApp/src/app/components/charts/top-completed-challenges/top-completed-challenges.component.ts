...
@Component({
    ...
    providers: [provideEchartsCore({ echarts }), PopupLoaderService]
})
export class TopCompletedChallengesComponent implements OnInit {
    ...
    fetchChartData(): void {
        this.netApi
            .get<{
                challenges: TopCompletedChallenges[]
            }>('Statistics', 'top-completed-challenges')
            .subscribe({
                next: (r) => this.loadChartData(r.challenges),
                error: () =>
                    this.popupLoader.showPopup(
                        'Erro ao carregar top desafios completados.'
                    )
            })
    }
