...
export class NetApiService {
    ...
    get<T>(controller: string, action: string, ...routeParams: string[]): Observable<T> {
        return this.http.get<T>(this.#buildUrl(controller, action, routeParams))
    }

    // eslint-disable-next-line  @typescript-eslint/no-explicit-any
    post<T>(controller: string, action: string, data: any, ...routeParams: string[]): Observable<T> {
        return this.http.post<T>(this.#buildUrl(controller, action, routeParams), data, {
